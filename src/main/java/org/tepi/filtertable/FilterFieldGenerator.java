...
@SuppressWarnings("serial") class FilterFieldGenerator implements Serializable {
    ...
    private final Map<AbstractField<?>, Object> customFields =
            new HashMap<AbstractField<?>, Object>();
    ...
    void initializeFilterFields() {
        ...
        if (owner.getFilterable() != null) {
            for (Object property : owner.getVisibleColumns()) {
                Object usedProperty = property;
                if (owner.getContainerPropertyIds().contains(usedProperty)) {
                    Component filter = createField(property, owner
                            .getContainerDataSource().getType(usedProperty));
