...
public class VCustomScrollTable extends FlowPanel implements HasWidgets,
     ...
     * changes causing {@link VCustomScrollTable.VScrollTableBody#rowHeight} to be discarded and
     * the height recalculated by {@link VCustomScrollTable.VScrollTableBody#getRowHeight(boolean)}
     ...
     * in method {@link VCustomScrollTable.TableHead#updateCellsFromUIDL(com.vaadin.client.UIDL)}.
     * <p/>
     * For internal use only. May be removed or replaced in the future.
     */
    ...
    public VCustomScrollTable() {
        ...
        //scrollBodyPanel.sinkEvents(Event.ONMOUSEOVER | Event.ONMOUSEOUT);
     ...
     * @param originalWidth The width in pixels of the column before the resize event
    ...
    public void updateColumnProperties(UIDL uidl) {
        ...
        int rowsInHeader = uidl.hasAttribute("complexcaptionrows") ?
                uidl.getIntAttribute("complexcaptionrows") :
                1;

        String leftTable = uidl.hasAttribute("lefttableuniqueid") ?
                uidl.getStringAttribute("lefttableuniqueid") :
                null;
        String rightTable = uidl.hasAttribute("righttableuniqueid") ?
                uidl.getStringAttribute("righttableuniqueid") :
                null;
        String myTable = uidl.hasAttribute("mytableiuniqueid") ?
                uidl.getStringAttribute("mytableiuniqueid") :
                null;


        UIDL vc = uidl.getChildByTagName("visiblecolumns");
        if (vc != null) {
            tHead.setSpan4Headers(null, uidl, rowsInHeader);
            //            tHead.setSpan4Headers(uidl.getStringArrayAttribute("vcolorder"), null, tHead.getRowsInHeader());
        ...
        if (scrollBody != null) {
            scrollBody.rebuildColGroup();
        }

/*
        if(myTable != null)
            scrollBody.setBindScrollId(myTable);
*/
        if (leftTable != null && rightTable != null) {
            tFoot.setBindScrollDiv(leftTable, rightTable);
        }
    }
    ...
    private final ScheduledCommand lazyScroller = new ScheduledCommand() {
        @Override
        public void execute() {
    ...
    /**
     * This method exists for the needs of {@link com.vaadin.client.ui.VTreeTable} only. Not part of
    ...
    private void updateHeader(String[] strings) {
        ...
        tHead.setSpan4Headers(strings, null, tHead.getRowsInHeader());
        int i;
        ...
        tHead.setVisibleCellsRightBorders();
    ...
    public void sizeInit() {
        ...
        while (headCells.hasNext()) {
            ...
            if (isNewBody || hCell.getWidth() == -1) {
                ...
                setColWidth(i, w,
                        hCell.isDefinedWidth()); //PSC                setColWidth(i, w, false);
            }
            i++;
        }
    ...
    public class RowRequestHandler extends Timer {
        ...
        public void run() {
            if (client.hasActiveRequest() || navKeyDown) {
                ...
                } else if (firstRowInViewPort - (int) (cache_rate * pageLength)
                        > firstToBeRendered) {
                ...
                if (lastReqRow > lastToBeRendered) {
                    ...
                    lastToBeRendered =
                            (firstRowInViewPort + pageLength + (int) (pageLength * cache_rate));
                ...
                if (reqFirstRow < firstToBeRendered
                        || (reqFirstRow > firstToBeRendered && (reqFirstRow < firstRendered
    ...
    public class UpperHeaderCell extends HeaderCell {
        public UpperHeaderCell bottomCell = null;
        public Byte colSpan = null;
        public Byte rowSpan = null;
        private UpperHeaderCell parentCell = null;
        private int level = 0;

        public UpperHeaderCell(String colId, String headerText, Byte rs, Byte cs, int level_) {
        ...
        public UpperHeaderCell getTopParent() {
            if (parentCell != null && !this.equals(parentCell)) {
                return parentCell.getParentCell();
            } else {
                return this;
            }
        }
        ...
        public void onBrowserEvent(Event event) {
            if (bottomCell != null && !bottomCell.equals(this)) {
                bottomCell.onBrowserEvent(event, this);
            } else {
        ...
        protected void updateStyleNames(String primaryStyleName) {
            ...
            if (colSpan != null) {
                addStyleName(primaryStyleName + "-header-cell-colspan");
            }
            if (rowSpan != null) {
                addStyleName(primaryStyleName + "-header-cell-rowspan");
            }
            String[] styles = tHead.styles.get(cid);
            if (styles != null) {
        ...
        public void setParentCell(UpperHeaderCell cell) {
            parentCell = cell;
        }

        public UpperHeaderCell getParentCell() {
            return parentCell;
        }

        public void patchParentAfterRemove(TableHead tableHead) {
            if (this.equals(this.parentCell) || this.parentCell == null) {
                return;
            }
            Widget w = parentCell;
            if (parentCell.colSpan > 1) {
                parentCell.colSpan--;
                DOM.setElementPropertyInt(w.getElement(), "colSpan", parentCell.colSpan.intValue());
            } else {
                tableHead.remove(w);
            }
            parentCell.patchParentAfterRemove(tableHead);
        }
        ...
        public String getFullCaption() {
            if (parentCell == null || this.equals(parentCell)) {
                ...
                return parentCell.getFullCaption() + ":" + caption;
            }
        }
    ...
    public class HeaderCell extends Widget {
        ...
        Element spanContainer = DOM.createSpan();
        ...
        public boolean definedWidthOld = false;
        public int definedWidthInt = 0;

        private float expandRatio = 0;

        private boolean sorted;

        private Element parentTr = null;

        public void setParentTr(Element element) {
            parentTr = element;
        }

        public Element getParentTr() {
            return parentTr;
        }
        ...
        public HeaderCell(String colId, String headerText) {
            ...
            captionContainer.appendChild(spanContainer);
         ...
         * {@link VCustomScrollTable.VScrollTableBody#getMaxIndent()}.
         *
        ...
        public boolean isDefinedWidth() {
            ...
            return definedWidthInt == 1 && width >= 0;
        }

        /**
         * This method exists for the needs of {@link com.vaadin.client.ui.VTreeTable} only.
         * <p/>
        ...
        public void setText(String headerText) {
            ...
            DOM.setInnerHTML(spanContainer, headerText);
        }
        ...
        @Override
        public void onBrowserEvent(Event event) {
            onBrowserEvent(event, null);
        }

        public void onBrowserEvent(Event event, UpperHeaderCell sourceCell) {
            if (sourceCell == null) {
                sourceCell = (UpperHeaderCell) this;
            }
            if (enabled && event != null) {
                if (isResizing
                        || event.getEventTarget().cast() == colResizeWidget
                        || event.getEventTarget().cast() == sourceCell.colResizeWidget) {
                    if (dragging
                    ...
                    if (sourceCell.equals(this)) {
                        ...
                        sourceCell.handleCaptionEvent(event);
                    }
        ...
        private void fireHeaderClickedEvent(Event event) {
            if (client.hasEventListeners(VCustomScrollTable.this,
                ...
                client.updateVariable(paintableId, "headerClickCaption", this.caption, true);
            }
        }
        ...
        public void onResizeEvent(Event event) {
            switch (DOM.eventGetType(event)) {
                case Event.ONMOUSEDOWN:
         ...
         *
         * @param includeIndent         - width should include hierarchy column indent if
        ...
        public String getCaption() {
            ...
            return DOM.getInnerText(spanContainer);
        }
         ...
         *
         * @param columnIndex column index hint, if -1 (unknown) it will be detected
         * @return
         */
    ...
    public class TableHead extends Panel implements ActionOwner {
        ...
        ArrayList<Widget> visibleComplexCells = new ArrayList<Widget>();

        HashMap<String, UpperHeaderCell[]> availableComplexCells =
                new HashMap<String, UpperHeaderCell[]>();
        ...
        ArrayList<Element> trList = new ArrayList<Element>();
        ...
        private void setSpan4Headers(String[] colOrder, UIDL uidlMain, int rows) {
            colOrder = colOrder == null ? uidlMain.getStringArrayAttribute("vcolorder") : colOrder;
            UIDL uidl = uidlMain == null ? null : uidlMain.getChildByTagName("visiblecolumns");
            rowsInHeader = rows;
            hTableWrapper.getStyle().setPropertyPx("height", 30 * rowsInHeader);
            this.getElement().getStyle().setPropertyPx("height", 30 * rowsInHeader);

            for (int i = 0; i < rowsInHeader; i++) {
                Element tr = null;
                if (trList.size() < rowsInHeader) {
                    tr = DOM.createTR();
                    trList.add(tr);
            ...
            if (uidl != null) {
                ComplexHeaders.createFromUIDL(this.captions, client, uidlMain);
                ...
                while (it.hasNext()) {
                    ...
                    String columnStyles[] = col.hasVariable("columnstyles") ? col.
                            getStringArrayVariable("columnstyles")
                            : null;
                    if (columnStyles != null) {
                        styles.put(cid, columnStyles);
                    }
                }
                colOrder = al.toArray(new String[0]);
            }
            rowSpans.clear();
            colSpans.clear();
        ...
        private HashMap<String, Byte[]> getColSpans(String[] colOrder,
            ...
            for (int x = 0; x < colOrder.length; x++) {
                ...
                for (int y = rowsInHeader - 2; y > -1; y--) {
                    if (x > 0) {
                        String[] hdrs = captions.getCaptions(colOrder[x], false);
                        //                    if (hdrs != null && complexColumnHeaders.getCaptions(colOrder[x - 1], false)[y].equals(hdrs[y])
                        if (hdrs != null && isContinueH(
                                captions.getCaptions(colOrder[x - 1], false), hdrs, y)
                    ...
                    if (y < rowsInHeader - 2 && (colspan.get(rowsInHeader - 2 - y))
                            .equals(Byte.valueOf((byte) 0))) {
                        colspan.add(new Byte((byte) 0));
                        continue;
                    }
                    Byte colspanvalue = new Byte((byte) 1);
                    for (int right = x + 1; right < colOrder.length; right++) {
                        String[] hdrs = captions.getCaptions(colOrder[right], false);
                        //                    if (hdrs != null && complexColumnHeaders.getCaptions(colOrder[x], false)[y].equals(hdrs[y])
                        if (hdrs != null && isContinueH(captions.getCaptions(colOrder[x], false),
                                hdrs, y)
        ...
        protected void updateStyleNames(String primaryStyleName) {
            ...
            hTableWrapper.addClassName(primaryStyleName + "-header");
            columnSelector.setClassName(primaryStyleName + "-column-selector");

            setStyleName(primaryStyleName + "-header-empty");
            addStyleName(primaryStyleName + "-header-wrap");

            for (UpperHeaderCell c[] : availableComplexCells.values()) {
                for (int i = 0; i < c.length; i++) {
                    c[i].updateStyleNames(primaryStyleName);
                }
            }
        }
        ...
        @Override
        public void clear() {
            for (String cid : availableComplexCells.keySet()) {
                removeCell(cid);
            }
            availableComplexCells.clear();
            availableComplexCells
                    .put(ROW_HEADER_COLUMN_KEY, getUpperRowHeaderCellGroup(rowsInHeader));
        }

        private UpperHeaderCell[] getUpperHeaderCellGroup(String cid, String complexCaption[]) {

            if (complexCaption == null) {
                return null;
            }
            ArrayList<UpperHeaderCell> arrayHeader = new ArrayList<UpperHeaderCell>();
            for (int i = 0; i < complexCaption.length; i++) {
                arrayHeader.add(new UpperHeaderCell(
                        cid,
                        complexCaption[i],
                        rowSpans.get(cid)[i],
                        colSpans.get(cid)[i],
            ...
            if (arrayHeader.size() == 0) {
                ...
                return arrayHeader.toArray(
                        new UpperHeaderCell[] {new UpperHeaderCell("", "", null, null, 0)});
            }
        }

        private UpperHeaderCell[] getUpperRowHeaderCellGroup(int height) {
            ArrayList<UpperHeaderCell> arrayHeader = new ArrayList<UpperHeaderCell>();
            for (int i = 0; i < height; i++) {
                arrayHeader.add(new UpperHeaderCell(ROW_HEADER_COLUMN_KEY, "", null, null, i));
            }
            if (arrayHeader.size() == 0) {
                return null;
            } else {
        ...
        private ComplexHeaders captions = new ComplexHeaders();
        ...
        public void updateCellsFromUIDL(UIDL uidl) {
            ...
            while (it.hasNext()) {
                final UIDL col = (UIDL) it.next();
                Set<String> s = col.getAttributeNames();
                Set<String> ss = col.getVariableNames();

                final String cid = col.getStringAttribute("cid");
                updated.add(cid);
                UpperHeaderCell[] c1 = getUpperHeaderCell(cid);
                String[] complexCaption = captions.getCaptions(cid, false);
                if (c1 == null) {

                    // FIXME              	Здесь обработать что колонка - заголовок ряда! вместо конструктора.
                    if (cid.equals(ROW_HEADER_COLUMN_KEY)) {
                        availableComplexCells.put(ROW_HEADER_COLUMN_KEY,
                                getUpperRowHeaderCellGroup(rowsInHeader));
                        c1 = getUpperHeaderCell(cid);
                    } else {
                        c1 = getUpperHeaderCellGroup(cid, complexCaption);
                        if (c1 != null) {
                            availableComplexCells.put(cid, c1);
                    ...
                    if (c1 != null) {
                        for (int i = 0; i < c1.length; i++) {
                            c1[i].setText(complexCaption[i]);
                        }
                    }
                }
                for (int i = 0; i < c1.length; i++) {
                    if (col.hasAttribute("sortable")) {
                        if (c1[i].colSpan == null || c1[i].colSpan.intValue() == 1) {
                            c1[i].setSortable(true);
                            if (cid.equals(sortColumn)) {
                                c1[i].setSorted(true);
                            } else {
                                c1[i].setSorted(false);
                            }
                        }
                    } else {
                        c1[i].setSortable(false);
                    }

                    if (col.hasAttribute("align")) {
                        c1[i].setAlign(col.getStringAttribute("align").charAt(0));
                    } else {
                        c1[i].setAlign(ALIGN_LEFT);
                    }
                }
            ...
            for (Iterator<String> cit = availableComplexCells.keySet().iterator(); cit
                    .hasNext(); ) {
                String cid = cit.next();
                if (!updated.contains(cid)) {
                    //                    removeCell(cid);
                    removeComplexCell(cid);
                    cit.remove();
        ...
        public void enableColumn(String cid, int index) {
            ...
            final UpperHeaderCell[] c1 = getUpperHeaderCell(cid);
            Byte rs[] = rowSpans.get(cid);
            HeaderCell bottomCell = c1[rowsInHeader - 1];
            if (rs != null) {
                for (int i = rowsInHeader - 1; i > -1; i--) {
                    if (rs[i] > 0) {
                        bottomCell = c1[i];
                        break;
                    }
            ...
            if (!bottomCell.isEnabled() || getHeaderCell(index) != bottomCell) {
                setHeaderCell(index, cid);
                if (initializedAndAttached) {
                    headerChangedDuringUpdate = true;
                }
            }
        ...
        public void setHeaderCell(int index, String cid) {
            final UpperHeaderCell[] cell1 = getUpperHeaderCell(cid);
            Byte cs[] = this.colSpans.get(cid);
            Byte rs[] = this.rowSpans.get(cid);
            ...
            UpperHeaderCell bottomCell = cell1[rowsInHeader - 1];
            if (rs != null) {
                for (int i = rowsInHeader - 1; i > -1; i--) {
                    if (rs[i] > 0) {
                        bottomCell = cell1[i];
            ...
            if (bottomCell.isEnabled()) {
                for (int i = 0; i < cell1.length; i++) {
                    if ((cs == null || cs[i].intValue() != 0) && (rs == null
                            || rs[i].intValue() != 0)) {
                        DOM.removeChild(trList.get(i), cell1[i].getElement());
                        orphan(cell1[i]);
                    }
                }
                visibleCells.remove(bottomCell);


            }

            if (index <= visibleCells.size()) {
                UpperHeaderCell parentCell = null;
                // insert to right slot
                for (int i = 0; i < cell1.length; i++) {
                    if (rs == null || rs[i] > 0) {
                        if (cs == null || cs[i].intValue() == 1 || (index == visibleCells.size()
                                && cs[i].intValue() > 1)) {
                            if (cs != null) {
                                DOM.setElementPropertyInt(cell1[i].getElement(), "colSpan",
                                        cs[i].intValue());
                                cell1[i].colSpan = cs[i];
                            }

                            if (rs != null && rs[i].intValue() > 1) {
                                DOM.setElementPropertyInt(cell1[i].getElement(), "rowSpan",
                                        rs[i].intValue());
                                cell1[i].colResizeWidget.getStyle()
                                        .setPropertyPx("height", 30 * rs[i].intValue());
                                cell1[i].captionContainer.getStyle()
                                        .setPropertyPx("height", 28 * rs[i].intValue());
                                cell1[i].captionContainer.getStyle()
                                        .setPropertyPx("lineHeight", 28 * rs[i].intValue());
                                cell1[i].spanContainer.getStyle()
                                        .setProperty("verticalAlign", "middle");
                                cell1[i].spanContainer.getStyle()
                                        .setProperty("display", "inline-block");
                                cell1[i].spanContainer.getStyle()
                                        .setProperty("lineHeight", "normal");
                                cell1[i].rowSpan = rs[i];
                            }

                            cell1[i].setParentTr(trList.get(i));
                            if (index == visibleCells.size()) {
                                DOM.appendChild(cell1[i].getParentTr(), cell1[i].getElement());
                            } else {
                                int ind = index - 1;
                                int realIndex = 0;
                                while (ind >= 0) {
                                    Byte barr[] = colSpans.get(
                                            ((UpperHeaderCell) visibleCells.get(ind)).cid);
                                    if (barr == null //��� ��������� ����
                                            || barr[i].intValue() > 0) {
                                        barr = rowSpans.get(
                                                ((UpperHeaderCell) visibleCells.get(ind)).cid);
                                ...
                                DOM.insertChild(cell1[i].getParentTr(), cell1[i].getElement(),
                                        realIndex);
                            }
                            adopt(cell1[i]);
                            visibleComplexCells.add(cell1[i]);
                            cell1[i].setParentCell(parentCell);
                            parentCell = cell1[i];
                        } else if (cs != null && cs[i].intValue() == 0) {
                            UpperHeaderCell parent = null;
                            int x = index - 1;
                            while (x >= 0) {
                                Byte barr[] =
                                        colSpans.get(((UpperHeaderCell) visibleCells.get(x)).cid);
                                if (barr == null
                                        || barr[i].intValue() > 0) {
                                    break;
                                }
                            ...
                            UpperHeaderCell carr[] =
                                    getUpperHeaderCell(((UpperHeaderCell) visibleCells.get(x)).cid);
                            if (carr != null) {
                                parent = carr[i];
                            }
                            int y = i;
                            if (!(parent.rowSpan == null || parent.rowSpan.intValue() > 0)) {
                                y = i - 1;
                                while (y >= 0) {
                                    Byte barr[] = rowSpans.get(
                                            ((UpperHeaderCell) visibleCells.get(x)).cid);
                                    if (barr == null
                                            || barr[y].intValue() > 0) {
                                        break;
                                    }
                            ...
                            carr = getUpperHeaderCell(((UpperHeaderCell) visibleCells.get(x)).cid);
                            Byte barr[] = colSpans.get(((UpperHeaderCell) visibleCells.get(x)).cid);
                            parent = carr[y];
                            parent.colSpan = barr[y];
                            DOM.setElementPropertyInt(parent.getElement(), "colSpan",
                                    parent.colSpan);
                            parentCell = parent;
                        } else if (cs != null && cs[i].intValue() > 1) {
                            UpperHeaderCell parent = null;
                            if (cell1[i].isEnabled()) {
                                parent = cell1[i];
                            } else {
                                int x = index;
                                UpperHeaderCell carr[] = getUpperHeaderCell(
                                        ((UpperHeaderCell) visibleCells.get(x)).cid);
                                if (carr != null) {
                                    parent = carr[i];
                                } else
                ...
                for (int i = 0; i < cell1.length; i++) {
                    cell1[i].bottomCell = bottomCell;
                }
                visibleCells.add(index, bottomCell);
            } else if (index == visibleCells.size()) {
                UpperHeaderCell parentCell = null;
                // simply append
                for (int i = 0; i < cell1.length; i++) {
                    if (cs != null && cs[i].intValue() == 0) {
                        int ind = index - 1;
                        while (ind >= 0) {
                            Byte barr[] =
                                    colSpans.get(((UpperHeaderCell) visibleCells.get(ind)).cid);
                            if (barr != null
                                    && barr[i].intValue() > 0) {
                                UpperHeaderCell carr[] = getUpperHeaderCell(
                                        ((UpperHeaderCell) visibleCells.get(ind)).cid);
                                if (carr != null) {
                                    if (!barr[i].equals(carr[i].colSpan)) {
                                        carr[i].colSpan = barr[i];
                                        DOM.setElementPropertyInt(carr[i].getElement(), "colSpan",
                                                carr[i].colSpan);
                                    }
                                }
                                break;
                    ...
                    if ((cs == null || cs[i].intValue() != 0) && (rs == null
                        ...
                        if (cs != null && cs[i].intValue() > 1) {
                            DOM.setElementPropertyInt(cell1[i].getElement(), "colSpan",
                                    cs[i].intValue());
                            cell1[i].setSortable(false);
                            cell1[i].colSpan = cs[i];
                        }
                        if (rs != null && rs[i].intValue() > 1) {
                            DOM.setElementPropertyInt(cell1[i].getElement(), "rowSpan",
                            ...
                            cell1[i].spanContainer.getStyle()
                                    .setProperty("display", "inline-block");
                            cell1[i].spanContainer.getStyle().setProperty("lineHeight", "normal");
                            cell1[i].rowSpan = rs[i];
                        }
                        cell1[i].setParentTr(trList.get(i));
                        DOM.appendChild(cell1[i].getParentTr(), cell1[i].getElement());
                        if (parentCell == null) {
                            if (i > 0) {
                                if (index == 0) {
                                    ...
                                    while (irs > 0) {
                                        irs--;
                                        if (rs[irs].intValue() > 0) {
                                            break;
                                        }
                                    }
                                    while (ics > 0) {
                                        ics--;
                                        if (colSpans.get((((UpperHeaderCell) visibleCells
                                                .get(ics)).cid))[irs].intValue() > 0) {
                                            break;
                                        }
                                    }
                                    parentCell = this.getUpperHeaderCell(
                                            ((UpperHeaderCell) visibleCells.get(ics)).cid)[irs];
                                }
                            } else {
                                parentCell = cell1[i];
                            }
        ...
        void setVisibleCellsRightBorders() {
            for (int x = 0; x < visibleCells.size(); x++) {
                final UpperHeaderCell[] cell1 =
                        getUpperHeaderCell(((UpperHeaderCell) visibleCells.get(x)).cid);
                Byte cs[] = this.colSpans.get(((UpperHeaderCell) visibleCells.get(x)).cid);
                if (cs == null) {
                    continue;
                }
                for (int y = 0; y < cell1.length - 1; y++) {
                    if (cell1[y].isEnabled()
                            && cs[y] > 1) {
                        cell1[y].bottomCell = ((UpperHeaderCell) visibleCells.get(x + cs[y] - 1));
        ...
        public HeaderCell getHeaderCell(String cid) {
            UpperHeaderCell[] c = availableComplexCells.get(cid);
            if (c == null) {
                return null;
            }
            Byte rs[] = rowSpans.get(cid);
            HeaderCell bottomCell = c[rowsInHeader - 1];
            if (rs == null) {
                return bottomCell;
            }
            for (int i = rowsInHeader - 1; i > -1; i--) {
        ...
        public UpperHeaderCell[] getUpperHeaderCell(String cid) {
            return availableComplexCells.get(cid);
        }
        ...
        public void moveCell(int oldIndex, int newIndex) {
            ...
            DOM.removeChild(hCell.getParentTr(), hCell.getElement());
            DOM.insertChild(hCell.getParentTr(), hCell.getElement(), newIndex);
        ...
        public boolean remove(Widget w) {
            if (visibleCells.contains(w)) {
                ...
                if (this.visibleComplexCells.contains(w)) {
                    visibleComplexCells.remove(w);
                }
                orphan(w);
                DOM.removeChild(DOM.getParent(w.getElement()), w.getElement());
                if (availableComplexCells != null) {
                    UpperHeaderCell c = (UpperHeaderCell) w;
                    c.patchParentAfterRemove(this);
                }
                return true;
            } else if (this.visibleComplexCells.contains(w)) {
                visibleComplexCells.remove(w);
                orphan(w);
                DOM.removeChild(DOM.getParent(w.getElement()), w.getElement());
                return true;
        ...
        public void removeComplexCell(String colKey) {
            final UpperHeaderCell[] c = getUpperHeaderCell(colKey);
            for (int i = 0; i < c.length; i++) {
                remove(c[i]);
            }
        }
        ...
        private void focusSlot(int index) {
            removeSlotFocus();
            if (index > 0) {
                Element child = trList.get(0).getChild(index - 1).getFirstChild().cast();
                ...
                Element child = trList.get(0).getChild(index).getFirstChild().cast();
                child.setClassName(VCustomScrollTable.this
                        .getStylePrimaryName() + "-resizer");
                child.addClassName(VCustomScrollTable.this
        ...
        private void removeSlotFocus() {
            ...
            if (focusedSlot == 0) {
                Element child = trList.get(0).getChild(focusedSlot).getFirstChild().cast();
                child.setClassName(VCustomScrollTable.this
                        .getStylePrimaryName() + "-resizer");
            } else if (focusedSlot > 0) {
                Element child = trList.get(0).getChild(focusedSlot - 1).getFirstChild()
                        .cast();
                child.setClassName(VCustomScrollTable.this
                        .getStylePrimaryName() + "-resizer");
            }
        ...
        class VisibleColumnAction extends Action {
            ...
            public VisibleColumnAction(String colKey) {
                ...
                this.colKey = colKey;
                if (tHead.getHeaderCell(colKey) instanceof UpperHeaderCell) {
                    caption = ((UpperHeaderCell) tHead.getHeaderCell(colKey)).getFullCaption();
                } else {
        ...
        public void disableAutoColumnWidthCalculation(HeaderCell source) {
            ...
            for (UpperHeaderCell cell1[] : availableComplexCells.values()) {
                Byte rs[] = this.rowSpans.get(cell1[0].cid);
                HeaderCell bottomCell = cell1[rowsInHeader - 1];
                if (rs != null) {
                    for (int i = rowsInHeader - 1; i > -1; i--) {
                        if (rs[i] > 0) {
                            bottomCell = cell1[i];
                ...
                UpperHeaderCell hca[] = this.getUpperHeaderCell(bottomCell.cid);
                for (int i = 0; i < hca.length; i++) {
                    hca[i].disableAutoWidthCalculation();
                }
                if (!source.equals(bottomCell)) {
                    columns.add(bottomCell);
                }
            }
    ...
    public class FooterCell extends Widget {
         ...
         * @param ensureDefinedWidth Ensures that the given width is not recalculated
         */
        public void setWidth(int w, boolean ensureDefinedWidth) {
                    ...
                    Scheduler.get().scheduleDeferred(new Command() {
                        ...
                        public void execute() {
                            ...
                            if (tdWidth < maxIndent
                                    && tFoot.visibleCells.indexOf(this)
    ...
    public class TableFooter extends Panel {
        ...
        Element headerTableBody = DOM.createTBody();
        Element tr = DOM.createTR();
        Element bindScrollDiv = DOM.createDiv();

        public void setBindScrollDiv(String leftTable, String rightTable) {
            DOM.appendChild(table, bindScrollDiv);
            DOM.setElementAttribute(bindScrollDiv, "onload",
                    "bindScroll('#" + leftTable + "', '#" + rightTable + "') ");
        }
    ...
    public class VScrollTableBody extends Panel {
        ...
        public void setBindScrollId(String myId) {
            DOM.setElementAttribute(table, "id", myId);
        }
        ...
        private void constructDOM() {
            ...
            colGroup = constructColGroup();
            table.insertFirst(colGroup);
        ...
        private Element constructColGroup() {
            Element element = DOM.createColGroup();
            for (int i = 0; i < visibleColOrder.length; i++) {
                Element styledColumn = DOM.createCol();
                String columnId = visibleColOrder[i];
                String[] columnStyles = tHead.styles.get(columnId);
                if (columnStyles != null) {
                    for (int sc = 0; sc < columnStyles.length; sc++) {
                        styledColumn.addClassName(columnStyles[sc]);
                    }
                }
                element.appendChild(styledColumn);
            }
            return element;
        }

        public void rebuildColGroup() {
            Element newColGroup = constructColGroup();
            if (colGroup != null) {
                table.removeChild(colGroup);
            }
            table.insertFirst(newColGroup);
            colGroup = newColGroup;
        }

        public void updateStyleNames(String primaryStyleName) {
        ...
        /**
         * This method exists for the needs of {@link com.vaadin.client.ui.VTreeTable} only. May be
         ...
         * @see {@link com.vaadin.client.ui.VScrollTable#getHierarchyColumnIndex()}
         */
        ...
        public class VScrollTableRow extends Panel implements ActionOwner {
            ...
            private final Map<TableCellElement, TooltipInfo> cellToolTips =
                    new HashMap<TableCellElement, TooltipInfo>();
                ...
                if (enabled && !touchEventHandled) {
                    ...
                    switch (type) {
                                     ...
                                     */
                                        ...
                                        if (!currentlyJustThisRowSelected) {
                                            ...
                                            toggleSelection();
                                                ...
                                                (isSingleSelectMode() || isMultiSelectModeSimple())
                         ...
                         */
                            ...
                            if (dragmode != 0 || actionKeys != null) {
                                new Timer() {
                                    ...
                                    @Override
                                    public void run() {
                                        TouchScrollDelegate activeScrollDelegate =
                                                TouchScrollDelegate
                        ...
                        case Event.ONMOUSEOUT:
                            //                            if (targetCellOrRowFound) {
                            //                                //client.updateVariable(paintableId, "mouseOut", "" + rowKey, false);
                            //                            }
                            //                            break;
                        case Event.ONMOUSEOVER:
                            //                            if (targetCellOrRowFound) {
                            //                                client.updateVariable(paintableId, "mouseOverRow", "" + rowKey, false);
                            //
                            //                                if (getElement() == targetTdOrTr.getParentElement()) {
                            //                                    // A specific column was clicked
                            //                                    int childIndex = DOM.getChildIndex(getElement(),
                            ...
                            //                                    client.updateVariable(paintableId, "mouseOverColumn", colKey, false);
                            //                                }
                            //
                            //                                MouseEventDetails details = MouseEventDetailsBuilder
                            //                                        .buildMouseEventDetails(event);
                            //
                            //                                client.updateVariable(paintableId, "onMouseOverEvent", details.toString(), true);
                            //
                            //
                            //                            }
                            break;
            ...
            private boolean isSignificantMove(Event event) {
                 ...
                 */
                Touch touch = event.getChangedTouches().get(0);
                if (Math.abs(touch.getClientX() - touchStartX)
                        > TouchScrollDelegate.SIGNIFICANT_MOVE_THRESHOLD) {
                    return true;
                }
                if (Math.abs(touch.getClientY() - touchStartY)
                        > TouchScrollDelegate.SIGNIFICANT_MOVE_THRESHOLD) {
                    return true;
                }
                return false;
             ...
             *
             * @param rowKey The generated row key
             */
    ...
    protected String buildCaptionHtmlSnippet(UIDL uidl) {
        ...
        if (uidl.hasAttribute("icon")) {
            s = "<img src=\"" + Util
                    .escapeAttribute(client.translateVaadinUri(uidl.getStringAttribute("icon")))
                    + "\"" +
                    (uidl.hasAttribute("icon-title") ?
                            " title=\"" + uidl.getStringAttribute("icon-title") + "\"" :
                            "") +
                    " class=\"v-icon\">" + s;
        }
        return s;
        ...
        public boolean equals(Object obj) {
            if (obj instanceof TableDDDetails) {
                ...
                return dropLocation == other.dropLocation
                        ...
                        && ((colkey != null && colkey.equals(other.colkey)) || (colkey == null
                        && other.colkey == null));
            }
            return false;
        }
